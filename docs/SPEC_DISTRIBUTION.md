...
Ship Budget Notes on **Google Play** and **FOSS stores** (IzzyOnDroid / later F-Droid) as the **same local vault** (photos yes, **OCR removed**), with an optional external **support / tip** link — no ads, no AdMob, no Play Billing required for v1.
...
2. **Same `applicationId`** `com.budgetnotes.app` (Izzy mirrors your signed APK; Play uses your AAB). Users should pick **one** store.
